...
public class SpinnerModelEditorDemo extends JApplet {
  ...
  public SpinnerModelEditorDemo() {
    ...
    JSpinner.DateEditor dateEditor =
      new JSpinner.DateEditor(spinnerDate, "MMM dd, yyyy");
    ...
    // Register and create a listener for spinnerDay
    ...
    // Register and create a listener for spinnerMonth
  ...
  // Update date spinner to synchronize with the other three spinners
  private void updateDate() {
    // Get current month and year in int
    int month = ((SpinnerListModel)spinnerMonth.getModel()).
        getList().indexOf(spinnerMonth.getValue());
    int year = ((Integer)spinnerYear.getValue()).intValue();

    // Set a new maximum number of days for the new month and year
    SpinnerNumberModel numberModel =
      (SpinnerNumberModel)spinnerDay.getModel();
