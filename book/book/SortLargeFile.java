...
public class SortLargeFile {
  ...
  private static void displayFile(String f) throws Exception {
    DataInputStream temp = new DataInputStream(
      new BufferedInputStream(new FileInputStream(f), BUFFER_SIZE));
    while (temp.available() > 0) {
      System.out.print(temp.readInt() + " ");
    }
    temp.close();
  }
}
